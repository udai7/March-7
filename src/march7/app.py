...
def render_query_results(response: AgentResponse):
  ...
  st.markdown("### Other Environmental Impacts")
  ...
  if response.recommendations:
      ...
      with st.expander(f"**{idx}. {rec.action}**", expanded=(idx == 1)):
        ...
        with col1:
          ...
          if water_red > 0 or energy_red > 0 or waste_red > 0:
            ...
            if waste_red > 0:
              st.write(f"- Waste reduction: **{waste_red:.3f} kg/day**")
    ...
    if len(response.recommendations) > 0:
      ...
      tab1, tab2, tab3 = st.tabs([" CO₂ Emissions", " Water Usage", " Energy"])
...
def render_feedback_interface(response: AgentResponse):
  ...
  with col3:
    if st.button(" Inaccurate", key="inaccurate_btn"):
...
def render_financial_calculator():
  ...
  fin_tab1, fin_tab2 = st.tabs(["Cost Savings", "Green Investment ROI"])
  with fin_tab1:
...
def render_cost_savings_calculator(calculator: FinancialCalculator):
  ...
  if calc_type == "Transport Savings":
    ...
    st.markdown("#### Food & Grocery Cost Savings")
    ...
    with col2:
      ...
      st.warning(f" {current_age}-year-old appliances use ~{int(energy_multiplier*100-100)}% more energy due to wear")
    ...
    st.markdown("#### Heating & Cooling Savings")
...
def render_utility_comparison(calculator: FinancialCalculator):
  ...
  if comparison_type == " General Utility Comparison":
  ...
  elif comparison_type == "❄ Cooling Systems":
    ...
    if st.button("Generate Audit Report", key="audit_report"):
      ...
      if hvac_age > 10:
        improvements.append(("❄ Replace HVAC System", hvac_factor * current_bill * 12))
      improvements.append((" LED Lighting", current_bill * 0.05 * 12))
      improvements.append((" Smart Thermostat", current_bill * 0.08 * 12))
...
def render_carbon_credit_calculator(calculator: FinancialCalculator):
  ...
  if st.button("Calculate Carbon Position", key="calc_carbon"):
    ...
    if credit.net_position > 0:
      ...
      st.warning(f" **Net Position:** You would owe **${abs(credit.net_position):,.2f}** in carbon taxes.")
    else:
      st.info("⚖ **Net Position:** Carbon neutral - emissions balanced by reductions!")
...
Be precise with the product names and prices you can read."""
    ...
    st.warning(" GROQ_API_KEY not set. Please set it in your .env file to enable image analysis.")
    return None
...
def render_manual_product_entry(scanner: ReceiptScanner):
  ...
  if st.session_state.manual_products:
    ...
    with col2:
      if st.button("🗑 Clear All", key="clear_products"):
        st.session_state.manual_products = []
        st.rerun()
...
def main():
  ...
  # Hide Streamlit default branding for production
  st.markdown("""
    <style>
...
def render_environmental_dashboard():
  ...
  dash_tab1, dash_tab2 = st.tabs(["Impact Calculator", "Activity Comparison"])
  with dash_tab1:
...
def render_impact_calculator():
  ...
  if st.button(" Calculate My Impact", key="calc_impact"):
    ...
    with col1:
      st.metric(" CO₂ Emissions", f"{total_co2:.1f} kg/day", 
...
def render_activity_comparison():
  ...
  comparison_category = st.selectbox(
    ...
    [" Transportation", " Food & Meals", " Household", "🛍 Shopping", "🎮 Entertainment"],
  ...
  elif comparison_category == " Food & Meals":
  ...
  elif comparison_category == "🛍 Shopping":
...
def render_goal_tracker():
  ...
  with col1:
    co2_target = st.number_input(" CO₂ Target (kg/day)", min_value=1.0, max_value=50.0, 
  ...
  with col2:
    ...
    waste_target = st.number_input("🗑 Waste Target (kg/day)", min_value=0.1, max_value=5.0, 
...
def render_footprint_analyzer():
  ...
  if st.button(" Analyze Footprint", key="analyze_fp"):
    ...
    with col1:
      st.metric(" Total Footprint", f"{total_co2:.1f} tons CO₂/year")
      ...
      st.write(f"🏳 {country} average: {country_avg} tons")
      
      if total_co2 < world_avg:
        st.success(f" {((world_avg - total_co2) / world_avg * 100):.0f}% below world average!")
      else:
        st.warning(f" {((total_co2 - world_avg) / world_avg * 100):.0f}% above world average")
    ...
    if flights_year > 2:
      suggestions.append(" Take one less flight per year → Save ~0.5 tons/year")
...
def render_eco_tips():
  ...
  tip_category = st.selectbox(
    "Select category",
    [" Home & Energy", " Transportation", " Food & Diet", "🛍 Shopping & Consumption", " Water Conservation", "🗑 Waste Reduction"],
    key="tip_category"
  )
  ...
  if tip_category == " Home & Energy":
    ...
    tips = [
      ...
      (" Install a smart thermostat", "Save 10-15% on heating/cooling, ~$200/year", "Easy"),
      ...
      ("☀ Install solar panels", "Save 50-100% on electricity, ~$1,500/year", "Advanced"),
      ("❄ Upgrade to Energy Star appliances", "Save 10-50% per appliance", "Medium"),
      ("🌬 Use ceiling fans instead of AC", "Save up to 40% on cooling costs", "Easy"),
  ...
  elif tip_category == "🛍 Shopping & Consumption":
    tips = [
      ("🛍 Bring reusable shopping bags", "Save 500+ plastic bags/year", "Easy"),
      ...
      ("🏷 Choose products with minimal packaging", "Reduce waste by 30%", "Easy"),
      (" Buy products with recycled content", "Support circular economy", "Easy")
    ]
    ...
    tips = [
      ...
      ("🌧 Install rain barrel for garden", "Save 5,000+ liters/year", "Medium"),
    ...
    tips = [
      (" Learn what can be recycled locally", "Increase recycling rate by 50%", "Easy"),
      ...
      (" Use cloth napkins", "Save 2,000+ paper napkins/year", "Easy"),
